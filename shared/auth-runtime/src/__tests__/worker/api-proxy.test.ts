...
describe("proxyFetch", () => {
  ...
  it("retries on 401 with DPoP-Nonce header before forcing a refresh", async () => {
    const ensureFresh = vi.fn().mockResolvedValue({ accessToken: "at1", tokenType: "DPoP" });
    (globalThis.fetch as any)
      .mockResolvedValueOnce(new Response("nonce", { status: 401, headers: { "dpop-nonce": "fresh-nonce" } }))
      .mockResolvedValueOnce(new Response("{}", { status: 200 }));
    const ctx = await makeDpopContext(await generateDpopKey());
    const res = await proxyFetch(cfg, ctx, { accessToken: "at1", tokenType: "DPoP", ensureFresh, onRefresh: () => {} }, { path: "/n", method: "GET" });
    expect(res.status).toBe(200);
  ...
  it.each([
    ...
    const ensureFresh = vi.fn().mockResolvedValue({ accessToken: "at", tokenType: "Bearer" });
    (globalThis.fetch as any).mockResolvedValue(new Response("boom", { status }));
    const ctx = await makeDpopContext(await generateDpopKey());
    await expect(
      proxyFetch(cfg, ctx, { accessToken: "at", tokenType: "Bearer", ensureFresh, onRefresh: () => {} }, { path: "/e", method: "GET" }),
  ...
  it("throws API_UNAUTHORIZED when refresh-and-retry still returns 401", async () => {
    ...
    (globalThis.fetch as any).mockResolvedValue(new Response("no", { status: 401 }));
    const ctx = await makeDpopContext(await generateDpopKey());
    await expect(
      proxyFetch(cfg, ctx, { accessToken: "at1", tokenType: "Bearer", ensureFresh, onRefresh: () => {} }, { path: "/u", method: "GET" }),
  ...
  it("returns empty body on 204 No Content", async () => {
    ...
    const ctx = await makeDpopContext(await generateDpopKey());
    const res = await proxyFetch(cfg, ctx, {
      accessToken: "at", tokenType: "Bearer",
      ensureFresh: async () => ({ accessToken: "at", tokenType: "Bearer" }),
      onRefresh: () => {},
