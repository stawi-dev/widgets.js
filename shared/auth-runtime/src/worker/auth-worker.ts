...
export interface WorkerCore {
  ...
  logout(): Promise<{ endSessionUrl?: string }>;
  destroy(): void;
...
export async function createWorkerCore(
  ...
  async function completeFedcm(idToken: string, expectedNonce?: string) {
    ...
    if (typeof expectedNonce === "string" && expectedNonce.length > 0) {
      if (claims.nonce !== expectedNonce) {
        throw new AuthError(
          "FEDCM_NONCE_MISMATCH",
          "FedCM id_token nonce mismatch",
        );
      }
    }
    const kp = await generateDpopKey();
  ...
  function buildEndSessionUrl(endpoint: string, idTokenHint?: string): string {
    const url = new URL(endpoint);
    url.searchParams.set("client_id", cfg.clientId);
    if (idTokenHint) url.searchParams.set("id_token_hint", idTokenHint);
    if (cfg.logoutRedirectUri)
      url.searchParams.set("post_logout_redirect_uri", cfg.logoutRedirectUri);
    return url.toString();
  }

  async function logout(): Promise<{ endSessionUrl?: string }> {
    cancelTimer();
    const d = await getDiscovery(cfg.idpBaseUrl, cfg.timeouts).catch(
      () => null,
    );
    const idTokenHint = session?.lastIdToken;
    const refreshToken = tokens?.refreshToken;
    const endSessionUrl = d?.end_session_endpoint
      ? buildEndSessionUrl(d.end_session_endpoint, idTokenHint)
      : undefined;
    ...
    if (d?.end_session_endpoint && session?.lastIdToken) {
      ...
      if (cfg.logoutRedirectUri)
        form.set("post_logout_redirect_uri", cfg.logoutRedirectUri);
    ...
    if (d?.revocation_endpoint && refreshToken) {
      const form = new URLSearchParams({
        client_id: cfg.clientId,
        token: refreshToken,
